...
from train_ner.process_data import CHUCK_TAGS_NUM
from train_status import TrainConf
from train_ner import process_data

cf = ConfigParser()  # 用来读取配置文件的包
...
class MyModel:
    ...
    def train(self):
        ...
        char_embedding = Embedding(
            ...
            trainable=True,
        )(char_input)  # (char_input)? 所使用的input
        ...
        # word embedding是直接进行concat的，并以此作为基础，不断concat其他内容。
        # 类似用了tuple或list的形式来存储，最后concatenate
        ...
        #  ================================= layers
        #  radical embedding + conv  选择-> 如果要加入radical一起训练：
        ...
        if self.conf.get('network') == TrainConf.cnn:
            ...
            # 将characters embedding经过卷积cnn后直接concat

        #   作为[conv+gru]的对比 即在characters embedding处使用bilstm：
        if self.conf.get('network') == TrainConf.bilstm:
            bilstm_char = Bidirectional(LSTM(LSTM_SIZE, return_sequences=True, dropout=0.5))(
                Dropout(0.2)(char_embedding))  # remember how to use bidirectional. Bidirectional(LSTM())
            concat_list.append(bilstm_char)
            # 将characters embedding经过bilstm后直接concat

        #   CONV+GRU for characters embedding -> Bi-GRU + Convolutional layer
        if self.conf.get('network') == TrainConf.conv_gru:
            # 使用了双向的gru 即bi-gru
            bigru_char = Bidirectional(GRU(GRU_SIZE, dropout=0.5, return_sequences=True))(
                Dropout(0.2)(char_embedding))
            # 然后将bi-gru的输出作为卷积层的输入
        ...
        concat = concatenate(concat_list)  # concat

# =============================== 用于tagger
        # We use bigru_crf in our code. And bilstm_crf for comparision.
        # BIGRU_CRF here
        if self.conf.get('tagger') == TrainConf.bigru_crf:
            bigru_or_lstm = Bidirectional(GRU(GRU_SIZE, dropout=0.5, return_sequences=True))(
                Dropout(0.2)(concat))

        # BILSTM_CRF here for comparision
...
def run(conf=None):
    ...
    print('char_embedding_data.shape:3191,200?', char_embedding_data.shape)
